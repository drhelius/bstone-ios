...
{
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::LoadTexture Loading texture: %s\n", pTexture->m_strName);
    ...
    if (pFile != NULL)
    {
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::LoadTexture size %dx%d\n", pTexture->m_iWidth, pTexture->m_iHeight);
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::LoadTexture PVR texture loaded: %s.pvr\n", pTexture->m_strName);

        return true;
    }
    else
    {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "@@@ TextureManager::LoadTexture Unable to load PVR: %s.pvr\n", pTexture->m_strName);
        ...
        if (spriteImage)
        {
            ...
            if (mipmaps)
            {
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::LoadTexture generating mipmaps...\n");
            ...
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::LoadTexture size %dx%d\n", pTexture->m_iWidth, pTexture->m_iHeight);
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::LoadTexture PNG texture loaded: %s.png\n", pTexture->m_strName);
            return true;
        }
        else
        {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "@@@ TextureManager::LoadTexture Unable to load texture: %s\n", pTexture->m_strName);
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "@@@ TextureManager::LoadTexture Defaulting to missing texture\n");
            ...
            if (spriteImage)
            {
                ...
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::LoadTexture missing_texture.png loaded\n");
                return true;
            }
            else
            {
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "@@@ TextureManager::LoadTexture Unable to load default missing texture: missing_texture.png\n");
...
GLboolean TextureManager::GenMipMap2D(GLubyte *src, GLubyte **dst, int srcWidth, int srcHeight, int *dstWidth, int *dstHeight, int level)
{
    ...
    *dst = (GLubyte*) malloc(sizeof(GLubyte) * texelSize * (*dstWidth) * (*dstHeight));
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::GenMipMap2D level %d: %dx%d\n", level, *dstWidth, *dstHeight);
...
{
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::GetTexture Searching for %s\n", strTextureName);
    ...
    {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::GetTexture Already in use\n");
    ...
    else
    {
        ...
        if (LoadTexture(temp, mipmaps))
        {
            ...
            if (result->second)
            {
                ...
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::GetTexture New texture inserted\n");
                return temp;
            }
            else
            {
                ///--- ya existía??? nunca debe entrar aqui
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "@@@ TextureManager::GetTexture FATAL ERROR @@@\n");
            }
        }
        else
        {
            ///--- no se pudo cargar la textura
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "@@@ TextureManager::GetTexture Unable to load texture %s\n", strTextureName);
        }
...
bool TextureManager::UnloadTexture(const char* strTextureName)
{
    ...
    {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::UnloadTexture Deleting texture: %s\n", itor->second->m_strName);
    ...
    {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "@@@ TextureManager::UnloadTexture The texture was not found\n");
        return false;
    }
}
...
int TextureManager::UnloadAll(void)
{
    ...
    {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "+++ TextureManager::UnloadAll Deleting texture: %s\n", i->second->m_strName);
